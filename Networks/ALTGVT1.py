...
# from Networks.MultiScaleAttention import *
...
class CPVTV2(PyramidVisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.detr = Transformer(d_model=1024, return_intermediate_dec=False)
        self.query_embed = nn.Embedding(1, 1024)
        self.count_token = nn.Parameter(torch.zeros(1, 1, 1024))
        self.pos_embed = nn.Parameter(torch.zeros(1, 1, 144))
        ...
        # self.MSA = MultiScaleAttention(1024, num_heads=8)
        self.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        B,C,H,W = x[3].shape
        x[1]=self.v1(x[1])
        out= self.detr(x[3],None,x[1].flatten(2).permute(2, 0, 1),self.pos_embed)
        mu = self.regression(x[2],out)
        B, C, H, W = mu.size()
...
def alt_gvt_large(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load('/opt/data/private/FGFDETR/alt_gvt_large.pth') # todo pass path as argument
